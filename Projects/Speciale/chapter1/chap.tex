...
\begin{proposition}
	Given a right Hilbert $A$-module $E$, then $X$ is a full left Hilbert $\mathbb{K}_A(E)$-module with respect to the action $T\cdot x = T(x)$ for $x \in E$ and $T \in \mathbb{K}_A(E)$ and $\mathbb{K}_A(E)$-valued inner product $_{\mathbb{K}_A(E)} \langle x,y \rangle = \Theta_{x,y}$ for $x, y \in E$. Moreover, the corresponding norm $\lv \cdot \rv_{\mathbb{K}_A(E)} = \lv _{\mathbb{K}_A(E)} \langle \cdot, \cdot \rangle \rv^{\frac12}$ coincides with the norm $\lv \cdot \rv_{A}$ on $E$.
\end{proposition}
\begin{proof}
