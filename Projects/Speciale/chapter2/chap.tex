...
\begin{proof}
	...
	\begin{align*}
		(\iota_G(r) f(s))^* = \Delta(s^{-1}) \alpha_r(f(r^{-1}s^{-1})^*),
	\end{align*}
	which can be used to show that $\iota_G(r)^* = \iota_G(r^{-1})$, so that $\iota_G(r) \in M(A \rtimes_\alpha G)$.
\end{proof}
