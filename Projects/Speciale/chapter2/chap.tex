...
\end{proof}
\begin{theorem}
	Let $G \acts \alpha A$ be a $C ^*$-dynamical system and let $\H$ be a Hilbert space. If $L \colon A \rtimes_\alpha G \to \mathbb{B}(\H)$ is a non-degenerate $*$-representation, then there exists a non-degenerate covariant representation $(\pi,u)$ of $G \acts \alpha A$ into $ \mathbb{B}(\H)$ such that $L= \pi \rtimes u$, and in fact it can be defined by
	\begin{align*}
...
\end{theorem}
\begin{proof}
	We already have seen the assignment $(\pi,u) \mapsto \pi \rtimes u$. For the converse, suppose that $L$ is a non-degenerate representation of $A \rtimes_\alpha G$. Let $\pi, u$ be defined by the above. \todo{finish easy proof}
\end{proof}

%But first, we need to include the full group $C^*$-algebra, $C^*(G)$, of $G$, into the above developed theory:
%\begin{lemma}
