...
\end{proof}
\begin{lemma}
	Let $A$ be a $G$-product and $f,g \in L^1(G) \cap L^2(G)$ and $x \in M(A)$. Then $ \lambda_f^* x \lambda_g$ is $\hat \alpha$-integrable. If we further assume that $x \in A$, then the function $t \mapsto I(\lambda_f^* x \lambda_g \lambda_f)$ is continuous with respect to the norm topology and $\lambda_f^* x \lambda_f - x \to 0$ whenever $f$ ranges through an approximate identity of $L^1(G)$ which is also in $L^2(G)$.
	\label{olpe1lemma2.7}
\end{lemma}
\begin{proof}
