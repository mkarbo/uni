...
\end{proof}
\begin{remark}	
	For the following, recall that the Fourier transformation embeds $L^1(G)$ into a dense $*$-subalgebra of $C_0(\hat G)$, and recall that we saw that $M(C_0(\hat G)) \cong C_b(\hat G)$ in chapter 1, and hence given a $G$-product $A$, the (induced) $*$-representation $\lambda \colon L^1(G) \to M(A)$ extends by continuity to a $*$-representation of $\hat \lambda \colon C_0(\hat G)\to M(A)$. Taking the multiplier on each side, we obtain a representation $\hat \lambda M(C_0(\hat G)) = C_b(\hat G) \to M(M(A)) = M(A)$.
\end{remark}
\begin{lemma}
