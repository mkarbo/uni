...
\end{example}
\begin{proof}
	It is easy to establish that $C(S^1)$ is $\Z$-simple, since the system is minimal (see e.g. \todo{add self} \cite[5.2]{glasner1976proximal}). For the converse, note that by the above, it suffices to show that some isotropy group is $0$. Let $f \colon e^{2 \pi i t} \mapsto e^{2 \pi i t}$, $t \in [0,1]$. Then $\alpha_k(f) = f$ if and only if $k = 0$, hence $\Z_{f} = \{0\}$ implying by \cref{olgootconnes} that $\hat \Z (\alpha) = S^1$, implying by \cref{olpe1thm} that $C(S^1) \rtimes_\alpha \Z$ is simple.
\end{proof}
