...
\begin{proof}
	...
	In particular $\psi$ is then a $*$-homomorphism: The multiplicative domain of $\psi$ contains the image of $\overline \pi(C(X))$ and by linearity and continuity it must be all of $C^*(\overline \pi (C(X)))$. Also, the range of $\psi$ is contained in the range of $\psi \circ \overline \pi$, which by injectivity is isomorphic to $C(X)$. 
